<?php
	...
	// Run processes JavaScript files via YUI Compiler Jar file
	if ($minify) {
		// Grab each file and check they exist
		$files = explode(",", $filelist);
		$scripts = '';
		...
		foreach ($files as $value) {
			if (file_exists($value.$extention)) {
				$scripts .= file_get_contents($value.$extention);
			}
		}
		
		// Set-up paths
		Minify_YUICompressor::$jarFile = $_SERVER['DOCUMENT_ROOT'].'/YUI/yuicompressor-2.4.2.jar';
	 	Minify_YUICompressor::$tempDir = $_SERVER['DOCUMENT_ROOT'].'/PHP/Concatenate/YUI Compiler/tmp';
		
		// Store optimised code in variable
	 	$code = Minify_YUICompressor::minifyJs(
	 		$scripts
	 		,array('nomunge' => true, 'line-break' => 1000)
	 	);
		
		// Write the optimised code to the file
		echo $code;
	}
